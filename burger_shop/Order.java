...
public class Order {
  ...
  private static String addItemOrder(Menu menu) { //addItemOrder method for exercise 2 that adds a specific burger from my Menu class based on user input.
    String burgerName = "";
    Scanner scanner = new Scanner(System.in);
  ...
  protected static boolean getToppingsMenu(Burger burger, Menu menu, int quantity) {
          ...
          if (null != topping) {
            ...
            try {
              ...
              if (quantity <= 0)
                ...
                System.out.println("Added " + topping.getToppingName() + " x" + quantity + " to your burger!");
              }
              topping = null;
            } catch (Exception e) {
  ...
  public static double orderFromMenu (String itemName, int quantity, Menu menu, OrderItem order){
      ...
      for (int i = 1; i <= quantity; i++) {
        ...
        if (burger instanceof Burger) { //show toppings menu if type of burger
          getToppingsMenu((Burger) burger, menu, quantity);
        }
        else if (burger instanceof Meal) { //show toppings menu if burger is an instance of meal
